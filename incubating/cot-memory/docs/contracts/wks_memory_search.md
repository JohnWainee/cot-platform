# `wks_memory_search` — contract

- **Server:** memory · **Kind:** read · **Gate:** none · **Egress:** none

> Impl-side mirror. The contract of record lives in `cot-platform`'s
> `mcp-hub/contracts/`; keep them in sync.
...
- `vault_uninitialized` → run `wks_memory_status`.
...
```json
{ "tool": "wks_memory_search", "args": { "query": "memory server sourcing decision", "scope": "both", "limit": 5 } }
```
